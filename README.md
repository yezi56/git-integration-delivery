...
- 功能或修复提交先推送到用户明确授权的既有远端源分支，再由 `origin/<source>` 合入 integration，并校验 merge commit 的来源父提交。
- 只有用户在当前请求中明确授权具体分支时才创建分支；已有脏工作区需要隔离时使用 detached worktree。
...
- 每次 commit 前审阅完整拟提交 diff，按严重度报告高风险；`Critical` 或 `High` 发现必须返回使用者处理。测试按改动风险分级，不默认在每次 commit 跑全量测试。
...
Skill 会先核对分支、工作区、远端引用、已有 MR、待交付批次 SHA，并按改动风险选择项目验证命令。准备环境交付时，它会在相应 integration worktree 中同步环境基线，并用 `--no-commit` 准备合并结果，确保 commit 前可以审阅完整 diff。

## 风险分级测试

- 每次 commit：检查完整 staged diff、空白错误、格式/静态检查，以及直接覆盖改动行为的最小测试集。
- integration 交付：在最终合并树上验证改动模块、直接调用方和相关契约；有 migration 时执行迁移门禁。
- 全量测试：用于权限/租户、迁移和破坏性数据操作、公共中间件、全局配置和依赖、共享契约、并发事务、复杂冲突、跨模块大改及 release candidate。
- 相同 Git tree、相同命令和等价环境的成功结果可以复用；只因 commit、push、MR 或 ref 变化不重复跑全量测试。integration 合并后的 tree 不同则重新选择适用验证。
- 精确 commit/tree 的必需 CI 已成功时，可作为全量验证证据；仍需明确报告未覆盖边界。
...
1. 确认用户授权的既有来源分支符合 `feat/<需求ID>`，并且本次是完整、可独立验证的批次；功能提交先推送该远端来源分支，回读并记录其 SHA。
...
4. 每个普通或 merge commit 前检查完整 staged diff、未纳入改动、契约兼容性和高风险面，并在干净 worktree 或隔离快照中按风险等级验证精确 staged 版本；`Critical` 或 `High` 发现阻断提交，没有阻断项时仍需当前请求已明确授权提交。
5. commit 后对比 `origin/<environment>...HEAD`，检查提交、预期 diff、空白错误与所选风险等级的验证结果；有迁移时执行迁移门禁。
6. integration merge commit 的第二父提交必须等于已记录的远端来源分支 SHA；验证后再推送，并回读 MR 的源分支、目标分支、标题、开放状态和描述格式。
...
└── git-integration-delivery/
    ...
    ├── references/risk-based-validation.md
